                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
						<form method="post">
						    ...
						    <table class="table">
						        ...
						        <c:forEach var="dis" items="${listAll}" varStatus="status">
						            <tr>
						                <td>${status.index + 1}</td>
						                <td><a href="disselectone?id=${dis.id}">${dis.title}</a></td>
						                <td>${dis.author}</td>
						                <td>${dis.day}</td>
						            </tr>
						        </c:forEach>
						 ...
						 <div>
						    ...
						    <c:if test="${not empty sessionScope.isValidPassword}">
						        <a href="dis/disinsertform" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">게시글 추가</a>
						    </c:if>
						    ${disinsertform }
						</div>
						</form>
