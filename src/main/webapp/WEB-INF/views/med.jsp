                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
						<form method="post">
						    ...
						    <table class="table">
						        ...
						        <c:forEach var="med" items="${listAll}" varStatus="status">
						            <tr>
						                <td>${status.index + 1}</td>
						                <td><a href="medselectone?id=${med.id}">${med.title}</a></td>
						                <td>${med.author}</td>
						                <td>${med.day}</td>
						            </tr>
						        </c:forEach>
						...
						<div>
						    ...
						    <c:if test="${not empty sessionScope.isValidPassword}">
						        <a href="med/medinsertform" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">게시글 추가</a>
						    </c:if>
						    ${medinsertform }
						</div>
						</form>
